...
namespace :remy do
  ...
  task :ssh, :rake_args do |task, options|
    ...
    user = Remy::Configuration::Chef.configuration.user || 'root'
    if ip_address = Remy::Configuration::Chef.determine_ip_addresses_for_remy_run(options[:rake_args]).try(:first)
